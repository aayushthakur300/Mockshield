...
// const WebcamFeed = ({ onViolation, onInterruption, onResume }) => {
...
//   // Timers & Throttles
//   const noFaceStartTime = useRef(0);
//   const lastBeepTime = useRef(0);
//   const lastViolationTrigger = useRef(0);

//   // --- AUDIO BEEP SYSTEM ---
//   const playBeep = () => {
//     const now = Date.now();
//     if (now - lastBeepTime.current < 1000) return;
...
//         handleBlock("Security System Failed");
//       }
//     };
//     loadModels();
//   }, []);
...
//         stream.getVideoTracks()[0].onended = () => handleBlock("Camera Disconnected");

//         setStatus("✅ Active");
//         handleResume();

//       } catch (err) {
//         handleBlock("Camera Access Denied");
//       }
//     };
...
//           setIsBlocked(true);
//           setStatus(`⛔ ${reason}`);
//           if (onInterruption) onInterruption(reason);
...
//       const now = Date.now();
//       if (now - lastViolationTrigger.current > 2000) {
//           lastViolationTrigger.current = now;
//           playBeep();
//           if (onViolation) onViolation(msg);
//       }
//   };

//   // 3. The 3D Head Pose Analysis Loop
//   useEffect(() => {
//     let intervalId;
...
//             // --- FACE FOUND ---
//             if (isBlocked && status.includes("Face Missing")) handleResume();
//             noFaceStartTime.current = 0;
...
//             if (horizontalRatio < 0.5) violationMessage = "Looking RIGHT ➡️";
//             else if (horizontalRatio > 2.0) violationMessage = "Looking LEFT ⬅️";
//             else if (verticalRatio < 0.35) violationMessage = "Looking UP ⬆️";
//             else if (verticalRatio > 0.75) violationMessage = "Looking DOWN ⬇️";

//             if (violationMessage) {
//                 setStatus(`⚠️ ${violationMessage}`);
...
//                 ctx.shadowBlur = 4;
//                 ctx.fillText(violationMessage, -canvasRef.current.width + 20, 50);
//                 ctx.restore();

//                 triggerViolation(violationMessage);
//             } else {
//                 if (!isBlocked) setStatus("✅ Focus Good");
//             }

//         } else {
//             // --- NO FACE FOUND ---
//             if (noFaceStartTime.current === 0) noFaceStartTime.current = Date.now();
//             const elapsed = Date.now() - noFaceStartTime.current;

//             // Warning Phase (1s - 20s)
//             if (elapsed > 1000 && elapsed < 20000) {
//                 const timeLeft = Math.ceil((20000 - elapsed)/1000);
//                 setStatus(`⚠️ Face Missing (${timeLeft}s)`);
...
//                 ctx.shadowColor = "black";
//                 ctx.shadowBlur = 4;
//                 ctx.fillText(`RETURN TO FRAME: ${timeLeft}s`, -canvasRef.current.width + 50, 240);
//                 ctx.restore();
//             }

//             // BLOCK + FLAG if > 20 Seconds
//             if (elapsed > 20000) {
//                 handleBlock("Face Missing > 20s");
//             }
//         }
//       }, 500); 
//     };
...
//   }, [modelLoaded, isBlocked, status]); 

//   return (
//     <div className={`relative w-full h-64 bg-black rounded-lg overflow-hidden border-4 ${isBlocked ? 'border-red-600' : 'border-green-500'}`}>
//       <video 
//         ref={videoRef} 
//         autoPlay 
//         muted 
//         playsInline 
//         className={`w-full h-full object-cover transform scale-x-[-1] ${isBlocked ? 'opacity-50' : 'opacity-100'}`} 
//       />
//       <canvas 
//         ref={canvasRef} 
...
//                   <h3 className="text-white font-bold text-xl uppercase">Session Halted</h3>
//                   <p className="text-red-400 font-mono text-sm mt-1">{status.replace('⛔', '')}</p>
//               </div>
//           </div>
//       )}

//       <div className="absolute top-2 left-2 px-3 py-1 bg-black/60 text-white text-xs font-mono rounded backdrop-blur-sm z-40">
//         <div className="flex items-center gap-2">
//             <div className={`w-2 h-2 rounded-full ${isBlocked ? "bg-red-500 animate-ping" : "bg-green-500"}`}></div>
//             {status}
//         </div>
//       </div>
//     </div>
