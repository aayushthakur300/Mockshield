...
const Dashboard = () => {
  ...
  // --- HELPER: Get Precise Formatted Score ---
  const getScoreColor = (score, disqualified) => {
      if (disqualified) return 'border-red-500 text-red-600 bg-white';
      if (score >= 80) return 'border-green-500 text-green-600 bg-white';
      if (score >= 50) return 'border-yellow-500 text-yellow-600 bg-white';
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 bg-white text-black">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Header */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 md:p-8 mb-8 flex flex-col md:flex-row justify-between items-center gap-6">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tighter text-black mb-1">
              Candidate <span className="text-red-600">Dashboard</span>
            </h1>
            {/* <p className="font-mono text-sm text-gray-500 font-bold">SYSTEM_READY // V2.0</p> */}
          </div>
          <div className="flex flex-col sm:flex-row gap-4 items-center">
             ...
             {history.length > 0 && (
                 <button 
                     onClick={handleClearAll}
                     disabled={isClearing}
                     className="bg-white text-red-600 border border-red-200 px-6 py-4 rounded-xl font-bold shadow-sm hover:bg-gray-50 hover:border-red-300 transition-all inline-flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                 >
                     {isClearing ? (
                         <><i className="fa-solid fa-spinner fa-spin"></i> Clearing...</>
                     ) : (
             ...
             <Link to="/setup" className="bg-white text-black border-2 border-black px-8 py-4 rounded-xl font-bold shadow-lg hover:shadow-2xl hover:scale-105 hover:bg-gray-50 transition-all inline-flex items-center gap-2">
                <i className="fa-solid fa-play"></i> Initiate New Session
             </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col items-center justify-center text-center">
                <span className="font-mono text-xs font-bold text-gray-400 uppercase mb-2">Total Sessions</span>
                <p className="text-5xl font-black text-black">{history.length}</p>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col items-center justify-center text-center">
                <span className="font-mono text-xs font-bold text-gray-400 uppercase mb-2">Avg Performance</span>
        ...
        {loading ? (
            ...
            <div className="space-y-4">
                {history.length === 0 ? (
                    <div className="bg-white p-16 text-center border-dashed border-2 border-gray-300 rounded-xl">
                        <div className="text-gray-300 text-6xl mb-4"><i className="fa-solid fa-folder-open"></i></div>
                        <h3 className="text-xl font-bold text-black mb-2">Database Empty</h3>
                        <p className="text-gray-500 mb-6 max-w-md mx-auto">
                            No interview records found. Initiate your first technical assessment to generate data points.
                        </p>
                    </div>
                    ...
                    history.map((item) => {
                      ...
                      return (
                        <div 
                          key={item.id} 
                          onClick={() => handleViewReport(item)} 
                          className={`bg-white p-6 rounded-xl shadow-sm border flex flex-col md:flex-row gap-6 items-start md:items-center group hover:shadow-md hover:bg-gray-50 transition-all cursor-pointer relative overflow-hidden ${
                              disqualified ? 'border-gray-200 border-l-4 border-l-red-500' : 'border-gray-200 border-l-4 border-l-blue-500'
                          }`}
                        >
                          ...
                          <div className="md:w-1/4 z-10">
                              ...
                              <h4 className="font-black text-lg leading-tight text-black group-hover:text-blue-600 transition-colors uppercase break-words">
                                  {getTopicDisplay(item)}
                              </h4>
                          ...
                          <div className="md:w-2/4 z-10">
                               <div className={`p-3 rounded border transition-colors ${
                                   disqualified ? "bg-white border-red-200" : "bg-white border-gray-200 group-hover:bg-gray-100"
                               }`}>
                                  {disqualified ? (
                                      <div className="flex flex-col gap-1">
                                          <div className="text-red-600 font-bold flex items-center gap-2">
                          ...
                          <div className="md:w-1/4 flex items-center justify-end gap-4 z-10">
                              ...
                              <button 
                                  onClick={(e) => handleDelete(e, item.id)}
                                  className="w-10 h-10 rounded-full bg-white border border-gray-200 text-gray-400 hover:text-red-600 hover:border-red-600 hover:bg-white transition-all flex items-center justify-center shadow-sm"
                                  title="Delete Record"
                              >
                                  <i className="fa-solid fa-trash"></i>
                              </button>
