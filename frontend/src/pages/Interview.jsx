...
// import { loadProctoringModels, detectProctoring } from '../utils/face-proctor';
...
//   const { questions = ["Error: No Questions Generated"], config = { topic: "Unknown", round: "General" } } = location.state || {};
...
//   // --- INTEGRITY SHIELD STATE ---
//   const [violations, setViolations] = useState(0);
//   const [showWarning, setShowWarning] = useState(false);
  
//   // Ref to prevent double-counting (The Fix for "Even Counting")
//   const lastViolationTime = useRef(0);

//   // --- INTERNAL LOGGER ---
//   const logSystemEvent = (msg) => {
//     const timestamp = new Date().toLocaleTimeString();
//     console.log(`%c[SYSTEM_TRACE] ${timestamp}: ${msg}`, 'color: #ef4444; font-weight: bold; font-family: monospace;');
//   };
...
//     // THE FIX: If the last violation was less than 2 seconds ago, IGNORE this one.
//     // This stops "Tab Switch" and "Blur" from firing at the same time and counting +2.
...
//     audio.play().catch(err => console.warn("Audio blocked (Interact with page first):", err));

//     // 2. Update State (Strictly +1)
//     setViolations(prev => prev + 1);
//     setShowWarning(true);

//     // 3. Log Event
//     logSystemEvent(`⚠️ INTEGRITY ALERT: ${reason}`);

//     // 4. Hide Red Warning Overlay after 3 seconds
//     setTimeout(() => setShowWarning(false), 3000);
//   };

//   // --- 1. SETUP SYSTEM LISTENERS (Tab Switch & Blur) ---
//   useEffect(() => {
//     logSystemEvent("Session Initialized.");
//     logSystemEvent(`Loaded ${questions.length} questions for ${config.topic}.`);

//     // A. Tab Switch
//     const handleVisibilityChange = () => {
//       if (document.hidden) {
//         handleViolation("User switched tabs / Minimized");
//       }
//     };

//     // B. Focus Loss (Clicking Desktop/Taskbar)
//     const handleFocusLoss = () => {
//         handleViolation("Window Focus Lost (Alt-Tab/Desktop)");
//     };
...
//   // --- 2. SETUP FACE PROCTORING (Head Movement) ---
//   useEffect(() => {
//     let intervalId;

//     async function startProctoring() {
//         // Load the 68-point models
//         await loadProctoringModels();
        
//         // Grab the video element from the WebcamFeed component logic
//         // Note: We use a slight delay to ensure WebcamFeed has mounted and stream is active
//         const videoElement = document.querySelector('video'); 
        
//         if (videoElement) {
//             intervalId = setInterval(() => {
//                 detectProctoring(videoElement, (msg) => {
//                     handleViolation(msg);
//                 });
...
//     // Start after a short delay to allow DOM to paint
//     const timer = setTimeout(startProctoring, 2000);
...
//   const toggleMic = () => {
//     if (!recognition) {
//       alert("Browser Speech API not supported.");
//       return;
//     }
//     if (isListening) {
//       recognition.stop();
//       setIsListening(false);
//       logSystemEvent("Microphone Deactivated.");
//     } else {
//       try {
//         recognition.start();
//         setIsListening(true);
//         logSystemEvent("Microphone Active. Listening...");
...
//     logSystemEvent(`Processing Answer ${currIndex + 1}...`);
...
//       logSystemEvent(`Moving to Question ${currIndex + 2}`);
//     } else {
//       // FINISH SESSION
//       logSystemEvent("Session Complete. Initiating Analysis...");
//       setProcessing(true);
      
//       try {
//         // 1. Get Analysis from AI
//         const feedbackRes = await evaluateSession(updatedAnswers);
//         logSystemEvent("AI Analysis Received.");
...
//         logSystemEvent("Data Persisted. Redirecting.");
...
//         // CLICK TO UNLOCK AUDIO (Required for Beep to work)
//         onClick={() => { new Audio(BEEP_URL).play().catch(()=>{}) }} 
//     >
      
//       {/* Warning Overlay */}
//       {showWarning && (
//         <div className="fixed top-0 left-0 w-full h-2 bg-red-600 z-50 animate-pulse" />
//       )}
...
//             <div className="hidden md:flex items-center gap-2 px-3 py-2 bg-white rounded border border-gray-200 shadow-sm">
//                 <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
//                 <span className="text-[10px] md:text-xs font-bold text-slate-600">LIVE FEED</span>
//             </div>
...
//         {/* LEFT COLUMN: Webcam & Question */}
//         <div className="w-full lg:w-1/3 flex flex-col gap-6 shrink-0">
          
//           <div className="card-panel p-2 shadow-xl border-slate-200 bg-white rounded-lg">
//              <WebcamFeed />
//           </div>
...
//                 <span className="text-xs font-bold text-gray-500 uppercase">Candidate Response Terminal</span>
...
//                       onClick={handleNext}
//                       disabled={processing}
//                       className="w-full sm:w-auto btn btn-primary px-8 md:px-10 py-3 md:py-4 shadow-red hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-slate-900 text-white rounded-lg font-bold flex justify-center items-center"
//                     >
//                       {processing ? (
...
//                       ) : (
//                           <span className="flex items-center gap-2">
//                               {currIndex === questions.length - 1 ? "FINALIZE SESSION" : "CONFIRM ENTRY"} 
...
//     }
//   }, [hasStarted, isDisqualified, navigate]);
...
//   // 1. Camera Denied Modal (Updated with Fix Instructions)
...
//                </span>
//              </li>
//              {/* --- ADDED BOLD HIGHLIGHTED HEAD MOVEMENT RULE --- */}
//              <li className="flex items-start gap-3">
...
//       if (saved) {
//         const parsed = JSON.parse(saved);
//         // Only restore if it's the same topic
//         if (parsed.config?.topic === propsData.config?.topic) {
//           return parsed;
//         }
//       }
...
//   // --- TERMINATION HELPER (NOW SAVES TO DB) ---
...
//     // 3. MANDATORY: SAVE DISQUALIFIED SESSION
//     try {
//         const failurePayload = {
//             questions: answers,
//             topic: config.topic || "Mock Interview (Terminated)", // Save Target Domain
//             totalScore: 0, // Mandatory Score
...
//         console.log("💾 Saving Terminated Mock Interview...", failurePayload);
...
//         </h2>
//         <p className="text-blue-300 mt-4 text-lg font-mono">Analyzing your responses...</p>
//     </div>
//   );
...
//                         onClick={handleNext}
//                         disabled={processing}
//                         className="flex-1 sm:flex-none btn btn-primary px-8 md:px-10 py-3 shadow-red hover:shadow-xl transition-all disabled:opacity-50 bg-slate-900 text-white rounded-lg font-bold flex justify-center items-center"
//                       >
//                         {processing ? (
//                             <span className="flex items-center gap-2">
...
//         hasStarted: false,
//         isDisqualified: false,
//         isSubmitted: false // NEW: Track submission status
//       };
//     }
...
//   // --- REFRESH / HARD RELOAD DETECTION ---
//   useEffect(() => {
//     // CRITICAL: If submitted, we DO NOT check for fullscreen or refresh penalties.
//     // We just want to recover the session and show the processing screen.
//     if (isSubmitted) {
//         setProcessing(true); // Restore processing overlay immediately
//         return;
//     }
...
//   // --- TERMINATION HELPER (NOW SAVES TO DB) ---
//   const terminateSession = useCallback(async (reason) => {
//     // If already submitted, do not allow termination (flags are frozen)
...
//     }
//     navigate('/'); 
//   }, [navigate, SESSION_KEY, answers, config.topic, violations, isSubmitted]);
...
//   // --- VIOLATION HANDLER ---
//   const handleViolation = useCallback((reason) => {
//     // 1. GLOBAL OVERRIDE: If submitted, STOP all violation counting.
//     if (isSubmitted) return;
...
//   // --- PRIVACY SHUTTER MONITOR ---
//   useEffect(() => {
//     if (isSubmitted) return; // Disable if submitted
//     if (!hasStarted || isDisqualified) return;
...
//   const handlePrevious = () => {
//     if (isSubmitted) return; // Lock nav
//     if (currIndex > 0) {
//         if (isListening) {
//             recognition.stop();
//             setIsListening(false);
...
//     // 1a. Force save to localStorage immediately to prevent data loss on crash/refresh
//     const lockingState = {
//         questions,
//         config,
//         currIndex,
...
//     } catch (err) {
//       console.error(err);
//       // NOTE: We do not unlock 'isSubmitted'. The user must wait or refresh (which triggers auto-retry).
//       alert("Error submitting interview. If this persists, refresh the page to retry automatically.");
//       // We keep processing=true so the user cannot change answers.
//     }
//   };

//   // --- AUTO-RETRY ON RELOAD ---
//   // If the user refreshed the page while "isSubmitted" was true,
//   // this effect will automatically trigger the finish logic again using the saved answers.
//   useEffect(() => {
...
//     <div 
//       className="min-h-screen flex flex-col p-4 md:p-6 bg-slate-50 relative overflow-x-hidden"
//       onClick={() => { if(!isSubmitted) new Audio(BEEP_URL).play().catch(()=>{}) }} 
//     >
//       {/* 0. PROCESSING OVERLAY */}
//       {processing && renderProcessingOverlay()}
...
//       {/* 2. INITIALIZATION SCREENS - Hide if submitted */}
...
//       {/* 3. RUNTIME MODALS - ONLY SHOW IF NOT SUBMITTED */}
...
//           <div className="card-panel p-2 shadow-xl border-slate-200 bg-white rounded-lg relative transition-all duration-300 z-0">
//               <WebcamFeed onViolation={handleViolation} />
//               {/* Hide REC indicator if submitted */}
//               {!isSubmitted && (
//                   <div className="absolute top-2 right-2 bg-red-600 text-white text-[10px] px-2 py-1 rounded animate-pulse font-bold">
//                     REC
//                   </div>
...
//                  spellCheck="false"
//                  disabled={isSubmitted} // LOCK TEXTAREA
//                />
//                <div className="absolute bottom-0 left-0 w-full bg-white/95 backdrop-blur-sm border-t border-gray-100 p-4 md:p-6 flex flex-col sm:flex-row justify-between items-center gap-4 z-10">
...
//                       <button 
//                         onClick={handlePrevious}
//                         disabled={currIndex === 0 || processing || isSubmitted} // LOCK PREV
//                         className="flex-1 sm:flex-none btn px-6 py-3 shadow-sm hover:shadow-md transition-all disabled:opacity-30 disabled:cursor-not-allowed bg-white border border-slate-300 text-slate-700 rounded-lg font-bold flex justify-center items-center"
//                       >
//                           <i className="fa-solid fa-arrow-left mr-2"></i> PREV
//                       </button>
...
//                       <button 
//                         onClick={handleNext}
//                         disabled={processing || isSubmitted} // LOCK NEXT/FINALIZE
//                         className="flex-1 sm:flex-none btn btn-primary px-8 md:px-10 py-3 shadow-red hover:shadow-xl transition-all disabled:opacity-50 bg-slate-900 text-white rounded-lg font-bold flex justify-center items-center"
//                       >
//                         {processing ? (
//                             <span className="flex items-center gap-2">
...
const Interview = () => {
  ...
  const renderDisclaimerModal = () => (
    <div className="fixed inset-0 bg-slate-900 z-[100] flex flex-col items-center justify-center p-2 sm:p-4 md:p-6 overflow-hidden">
      <div className="bg-white max-w-2xl w-full max-h-[95vh] flex flex-col rounded-xl shadow-2xl p-4 sm:p-6 md:p-10 relative">
        <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tight mb-3 sm:mb-6 border-b-4 border-blue-600 pb-2 sm:pb-4 shrink-0">
            Interview Guidelines
        </h2>
        <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-8 text-slate-700 text-sm sm:text-lg leading-snug sm:leading-relaxed overflow-y-auto flex-1 pr-1 sm:pr-2">
           <ul className="list-none space-y-2 sm:space-y-3 bg-slate-50 p-3 sm:p-6 rounded-lg border border-slate-200">
             ...
             <li className="flex items-start gap-2 sm:gap-3">
               ...
               <span>
                  <strong>Face Visibility</strong>: If your face is not in the frame for <strong>20 seconds</strong>, a flag will be triggered.
                  <span className="block text-red-600 text-xs sm:text-sm mt-1 font-bold">
                      ⚠️ If this cycle repeats 3 times, you will be DISQUALIFIED immediately.
                  </span>
               </span>
             </li>
             ...
             <li className="flex items-start gap-2 sm:gap-3 text-red-600 font-bold border-l-4 border-red-500 pl-2 sm:pl-3 bg-red-50 py-1 sm:py-2">
               <i className="fa-solid fa-triangle-exclamation mt-1"></i>
               <span>
                  CRITICAL: If your Flag Count reaches {MAX_VIOLATIONS}, you will be IMMEDIATELY DISQUALIFIED.
               </span>
        ...
        <label className="flex items-center gap-2 sm:gap-3 cursor-pointer p-2 sm:p-4 hover:bg-slate-50 rounded-lg transition-colors border border-transparent hover:border-slate-200 shrink-0">
          <input 
            type="checkbox" 
            className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 rounded focus:ring-blue-500"
            checked={disclaimerAccepted}
            onChange={(e) => setDisclaimerAccepted(e.target.checked)}
          />
        ...
        <button 
          onClick={handleStartInterview}
          disabled={!disclaimerAccepted}
          className={`w-full mt-3 sm:mt-6 py-3 sm:py-4 rounded-lg font-bold text-base sm:text-xl uppercase tracking-wider transition-all shadow-xl shrink-0 ${
            disclaimerAccepted 
            ? 'bg-blue-600 hover:bg-blue-700 text-white hover:scale-[1.02]' 
            : 'bg-gray-300 text-gray-500 cursor-not-allowed'
          }`}
  ...
  const renderPrivacyShutterModal = () => {
    ...
    We detected your camera
    privacy shutter is closed or
    the room is too dark.

    Open the shutter
    immediately.
    The session will terminate in
    119s if not resolved.
