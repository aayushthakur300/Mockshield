...
const ResumeReport = () => {
  ...
  const handleDownload = () => {
    ...
    processedData.question_reviews.forEach((item, index) => {
        ...
        // --- FIXED QUESTION BOX: Now wraps full question text dynamically ---
        const qText = `Q${index + 1}: ${item.question || "Question Text Missing"}`;
        doc.setFontSize(10);
        const splitTitle = doc.splitTextToSize(qText, contentWidth - 35);
        const boxHeight = (splitTitle.length * 4.5) + 6; 

        doc.setFillColor(241, 245, 249);
        doc.rect(margin + 2, cursorY, contentWidth - 4, boxHeight, 'F');
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(51, 65, 85);
        
        doc.text(splitTitle, margin + 5, cursorY + 6);
        doc.text(`Score: ${item.score}/10`, pageWidth - margin - 25, cursorY + 6);
